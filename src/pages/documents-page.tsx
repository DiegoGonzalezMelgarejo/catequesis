...
import {
  ...
  getDocumentRequirementCatalog,
  saveDocumentRequirementCatalog,
  type DocumentRequirementEntry,
...
export function DocumentsPage() {
  ...
  const { data, loading } = useAsyncData(
    () => (user ? getDocumentRequirementCatalog() : Promise.resolve(null)),
    [user?.id],
  )
  const [items, setItems] = useState<DocumentRequirementEntry[]>([])
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <AppCard
        ...
        description="Crea cada documento una sola vez y asígnalo a uno o varios sacramentos."
        actions={
          <PrimaryButton
            type="button"
            onClick={async () => {
              try {
                setSaving(true)
                await saveDocumentRequirementCatalog(user, items)
                toast.success('Documentos requisito guardados.')
              } catch (error) {
      ...
      >
        <p className="text-sm text-muted-foreground">
          Los alumnos verán automáticamente estos requisitos según los sacramentos que tengan asignados.
        </p>
      </AppCard>

      {data.sacraments.length === 0 ? (
        <EmptyState title="Sin sacramentos" description="No hay sacramentos disponibles para crear requisitos." icon={FileCheck} />
      ) : (
        <AppCard title="Lista de documentos" description="Puedes asignar cada documento a uno o varios sacramentos.">
          <div className="space-y-4">
            {items.length === 0 ? (
              <EmptyState title="Sin documentos" description="Agrega el primer documento requisito." icon={FileCheck} />
            ) : (
            ...
            <SecondaryButton
              type="button"
              onClick={() => {
                setItems((current) => [...current, createDocumentRequirementItem()])
              }}
            >
